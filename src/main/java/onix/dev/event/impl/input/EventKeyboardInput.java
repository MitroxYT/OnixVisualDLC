...
public class EventKeyboardInput extends Event {
    ...
    public void setYaw(float yaw, float yaw2) {
        ...
        double angle = Mth.wrapDegrees(Math.toDegrees(direction(yaw2, forward, sideways)));
        ...
        for (float predictedForward = -1F; predictedForward <= 1F; predictedForward += 1F) {
            for (float predictedSideways = -1F; predictedSideways <= 1F; predictedSideways += 1F) {
                ...
                double predictedAngle = Mth.wrapDegrees(Math.toDegrees(direction(yaw, predictedForward, predictedSideways)));
